...
export function AppShell({ children, user, onLogout }: AppShellProps) {
  ...
  return (
    <div className="l-app-shell">
      <aside className={`l-app-shell__sidebar ${mobileMenuOpen ? 'l-app-shell__sidebar--open' : ''}`}>
        ...
        <nav className="c-nav" aria-label="主要導覽">
          {visibleNavigationItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => `c-nav__item ${isActive ? 'c-nav__item--active' : ''}`}
            >
              <span className="c-nav__icon" aria-hidden="true">{item.icon}</span>
              {item.label}
            </NavLink>
          ))}
        </nav>

        <button className="c-button c-button--ghost c-button--full" type="button" onClick={onLogout}>登出</button>
      </aside>

      {mobileMenuOpen && (
        <button type="button" aria-label="關閉選單" className="l-app-shell__backdrop" onClick={() => setMobileMenuOpen(false)} />
      )}

      <div className="l-app-shell__content">
        <header className="l-app-shell__topbar">
          <button className="c-icon-button l-app-shell__menu-button" type="button" aria-label="開啟選單" onClick={() => setMobileMenuOpen(true)}>☰</button>
          <span className="l-app-shell__topbar-title">{user.display_name}</span>
          <span className="c-live-dot">{user.is_admin ? '系統管理員' : '個人帳號'}</span>
        </header>
